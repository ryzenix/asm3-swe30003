    ...
    async addToCart(req, res, next) {
        try {
            ...
            const { rows: products } = await this.db.query(
                'SELECT id, title, price_value, stock_quantity, requires_prescription, images, main_image_index, manufacturer, category FROM products WHERE id = $1 AND is_active = TRUE',
                [productId]
            );
    ...
    async validateCartItems(cart) {
        try {
            ...
            for (const item of cart.items) {
                ...
                const { rows: products } = await this.db.query(
                    'SELECT id, title, price_value, stock_quantity, requires_prescription, images, main_image_index, manufacturer, category, is_active FROM products WHERE id = $1',
                    [item.id]
                );

                if (products.length === 0 || !products[0].is_active) {
                    // Product no longer exists or is inactive - skip it
                    this.logger.warn(`Product ${item.id} no longer available, removing from cart`);
                    continue;
                }
