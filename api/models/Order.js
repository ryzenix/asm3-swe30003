...
class Order {
            ...
            try {
                ...
                const { rows: newOrder } = await client.query(
                    `INSERT INTO orders (
                        ...
                        notes, estimated_delivery_date, prescription_required
        ...
        try {
            ...
            let query = `
                SELECT 
                    ...
                    o.notes, o.estimated_delivery_date, o.prescription_required, o.created_at, o.updated_at,
        ...
        try {
            ...
            const ordersQuery = `
                SELECT 
                    ...
                    o.shipping_method, o.prescription_required, o.created_at, o.updated_at,
                    u.email as user_email, u.full_name as user_name, u.phone as user_phone,
    ...
    async cancel(orderId, userId, reason = null) {
        try {
            if (!orderId) {
                throw ValidationError.missingFields(['orderId']);
            }
            ...
            try {
                ...
                await client.query(
                    ...
                    ['cancelled', reason || 'Cancelled by customer', orderId]
                );

                await client.query('COMMIT');

                this.logger.info(`Order cancelled - ID: ${orderId}, User: ${userId}, Reason: ${reason}`);
